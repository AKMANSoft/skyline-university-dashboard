...
import EditLocTypeModal from "./modals/EditLocationModal";
...
const LocationType = () => {
  ...
  return (
    <>
    <EditLocTypeModal modalType="Edit" open={openEdit} handleClose={handleCloseEdit} />
    <Table sx={{ borderTop: "1px solid #DBDADE" }}>
      <TableHead>
        <TableRow>
          <TableCell>
            <TableSortLabel
              active={sortBy === "locType"}
              direction={sortBy === "locType" ? sortDirection : "asc"}
              onClick={() => handleSort("locType")}
              IconComponent={() => (
                <SortIcon active={sortBy === "locType"} direction={sortDirection} />
              )}
              sx={{
                fontSize: "13px",
                fontWeight: 600,
