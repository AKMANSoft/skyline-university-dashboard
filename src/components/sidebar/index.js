    ...
    const handleMenuClick = (type, index) => {
        if(type==="main"){
            ...
            if (openMenus.subMenus.includes(index)) {
                setOpenMenus({...openMenus, subMenus: openMenus.subMenus.filter((item) => item !== index)});
            }else{
                setOpenMenus({...openMenus, subMenus: [...openMenus.subMenus, index]});
            }
        }
    };
        ...
        return submenus?.map((submenu, subIndex) => (
            <div key={subIndex}>
                ...
                >
                    ...
                    />
                    {submenu.children && (openMenus?.subMenus?.includes(parentIndex + '.' + subIndex) ? 
                        <ExpandMore sx={{color: '#696969'}} /> : 
                        <ChevronRightIcon sx={{color: '#696969'}} />
                    )}
                </ListItem>
                {submenu.children && (
                    <Collapse in={openMenus?.subMenus?.includes(parentIndex + '.' + subIndex)} timeout="auto" unmountOnExit>
                        <List component="div" disablePadding>
                            {renderSubMenuItems(submenu.children, depth + 1, parentIndex + '.' + subIndex)}
                        </List>
                    </Collapse>
                )}
            </div>
        ...
        return menuItems?.map((menuItem, index) => (
            ...
            >
                <Typography variant='p' component='p' 
                    sx={{fontSize: '11px', clor: '#4B465C', lineHeight: '14px', mb: 1}}>
                    Create & Add
                </Typography>
            </Box>
            :
            <div key={index}>
                <ListItem 
                    ...
                    sx={{
                        ...
                        pr: '4px',
                        mb: 1,
                        background: openMenus.mainActive===index ? 'linear-gradient(72deg, #00318B 22.16%, #2C549E 76.47%)' : 'inherit',
                        borderRadius: '6px',
                        '&:hover': {
                            background: openMenus.mainActive===index ? 'linear-gradient(72deg, #00318B 22.16%, #2C549E 76.47%)' : 'rgba(0, 0, 0, 0.04)'
                        },
                    }}
                >
                    <ListItemIcon 
                    ...
                    />
                    {menuItem.children && (openMenus.mainActive===index ? 
                        <ExpandMore sx={{color: openMenus.mainActive===index ? 'white' : '#696969'}} /> : 
                        <ChevronRightIcon sx={{color: openMenus.mainActive===index ? 'white' : '#696969'}} />
                    )}
                </ListItem>
                {menuItem.children && (
                    <Collapse in={openMenus.mainActive===index} timeout="auto" unmountOnExit>
